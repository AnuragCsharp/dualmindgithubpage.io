...
import footer_logo from "@/assets/img/logo/logo.svg";
...
// TODO: Inherit Data and Use Cenral SieInfo
const footer_data: DataType = {
  email: site_info.email,
  phone: site_info.phone,
  location: site_info.address,
  footer_info: site_info.footer_info,
  links: [
    { title: `HOME`, link: "/" },
...
const FooterOne = () => {
  return (
    <>
      <footer className="tl_footer tl_primary_bg">
        <div className="tl_height_150 tl_height_lg_60"></div>
        <div className="container">
          <div className="row">
            <div className="col-lg-5">
              <div className="tl_footer_info">
                <Image src={footer_logo} alt="Logo" />
                <p>{footer_info}</p>
                <a href={`mailto:${email}`}
                  className="tl_primary_font anim_text_upanddowns"><span>{email}</span></a>
              </div>
            </div>
            <div className="col-lg-6 offset-lg-1">
              <div className="tl_footer_social">
                <SocialLinks />
              </div>
              <div className="tl_height_60 tl_height_lg_30"></div>
              <ul className="tl_footer_contact_list tl_mp0">
                <li>
                  <i>
          ...
          <div className="tl_height_90 tl_height_lg_60"></div>
          <ul className="tl_footer_nav">
            {links.map((item, index) => (
              <li key={index}><Link href={item.link}>{item.title}</Link></li>
            ))}
          </ul>
        </div>
        <div className="tl_copyright text-center">
